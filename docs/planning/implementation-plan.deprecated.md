...
## IMPORTANT:
> All work picked up by agents should be performed in the `agent-dev` branch.  
...
- Publish a release‑pack consumable by `Cannasol-Tech/functionality-reports` per `docs/release/release-standard.md` (spec TBD; placeholder implemented in MVP).
...
- Functionality Reports compliance: releases include artifacts per `docs/release/cannasol-release-standard.md` (at minimum `final/executive-report.json`; optional `final/coverage-summary.json`, `final/unit-test-summary.json`, `final/executive-report.md`). Artifacts validate against schemas and are uploaded to the GitHub Release with correct content types.

  #2.1: SWA GitHub Actions deploy (tune `.github/workflows/azure-static-web-apps-*.yml`) [ ] (est: 0.25d) **[CHECKED OUT: cascade-04 @2025-08-27T11:34:21-04:00] [agent-dev]**
  ...
  #2.5: Functionality Reports release compliance [ ] (est: 0.5d) **[CHECKED OUT: cascade-01 @2025-08-27T11:35:00-04:00] [feature/release-standard-compliance]**
      2.5.1 [ ] Implement CI steps to generate required artifacts
            - Required: `final/executive-report.json` (acceptance results)
            - Optional: `final/coverage-summary.json`, `final/unit-test-summary.json`, `final/executive-report.md`
      2.5.2 [ ] Validate artifacts against schemas referenced in `docs/release/cannasol-release-standard.md`
      2.5.3 [ ] On tag push, create/update GitHub Release and upload `final/*` assets with correct content types
      2.5.4 [ ] On PRs, upload the same JSONs as workflow run artifacts (no release) for preview
      2.5.5 [ ] Add Makefile helper targets (optional): `make acceptance-report`, `make coverage-json`
      2.5.6 [ ] Document the process in `README.md` Testing/Release sections
      2.5.V [ ] Verification — CI run shows artifacts attached to a test Release; API consumer can ingest without errors
      2.5.T [ ] Tests — schema validation step passes in CI; add unit test(s) for JSON shaping utilities if present
      2.5.C [ ] Commit and push your work: `git add -A && git commit -m "2.5: release standard compliance" && git push`
