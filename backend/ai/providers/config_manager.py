...
class ProviderConfigManager:
    ...
    def _load_from_environment(self) -> None:
        ...
        for provider_type, defaults in self.DEFAULT_CONFIGS.items():
            ...
            logger.info(
                f"{provider_type.value}: {key_status}, model={model_name}, enabled={enabled}"
            )
    ...
    def create_provider(self, provider_type: ProviderType) -> Optional[LLMProvider]:
        ...
        try:
            ...
            provider = provider_class(
                api_key=api_key, base_url=credentials.base_url, **credentials.metadata
            )
            logger.info(
                f"Created {provider_type.value} provider with model {credentials.model_name}"
            )
            return provider
