...
class ConversationManager:
    ...
    def _archive_conversation(self, session_id: str):
        ...
        try:
            ...
            if session_file.exists():
                ...
                archive_file = archive_dir / f"{session_id}_{datetime.now().strftime('%Y%m%d')}.json"
                session_file.rename(archive_file)
                
        except Exception as e:
            logger.warning(f"Failed to archive conversation {session_id}: {e}")
    
    async def get_or_create_conversation(self, session_id: str, user_id: str) -> ConversationContext:
    ...
    async def add_message(self, session_id: str, role: str, content: str, 
        ...
        if len(context.messages) > self.max_context_length:
            ...
            recent_messages = [msg for msg in context.messages if msg.role != 'system'][-self.max_context_length:]
    ...
    async def _archive_conversation_async(self, session_id: str):
        ...
        await asyncio.get_event_loop().run_in_executor(
            None, self._archive_conversation, session_id
        )
