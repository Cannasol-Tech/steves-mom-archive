...
def require_roles(req: HttpRequest, required_roles: List[str]) -> UserPrincipal:
    ...
    if not user.has_any_role(required_roles):
        logger.warning(f"User {user.user_details} lacks required roles: {required_roles}")
...
def require_all_roles(req: HttpRequest, required_roles: List[str]) -> UserPrincipal:
    ...
    if not user.has_all_roles(required_roles):
        logger.warning(f"User {user.user_details} lacks all required roles: {required_roles}")
        raise ValueError(f"All required roles needed: {required_roles}")
    
    return user


def create_auth_response(status_code: int, message: str, user: Optional[UserPrincipal] = None) -> HttpResponse:
